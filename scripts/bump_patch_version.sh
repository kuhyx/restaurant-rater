...
has_non_pubspec_changes() {
    ...
    # `grep -q -v` rather than testing the output of a plain `grep -v`:
    # identical semantics (exit 0 when some line does not match), one fewer
    # subshell, and it is what shellcheck SC2143 asks for.
    grep -q -v -e '^$' -e 'pubspec\.yaml$' <<<"$staged"
}
